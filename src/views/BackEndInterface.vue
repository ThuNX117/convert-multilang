<template>
    <div class="backend-interface">
        ...
        <div class="json-inputs-container">
            <!-- Validation Summary -->
            <div v-if="jsonInputs.some(input => input.value)" class="validation-summary" :key="validationKey">
                <n-card title="Input Status Summary" size="small">
                    <div class="validation-grid">
                        <div v-for="(input, index) in jsonInputs" :key="`${index}-${input.isValid}`"
                            class="validation-item">
                            <div class="language-label">{{ getLanguageName(index) }}</div>
                            <div class="validation-status">
                                <n-tag v-if="!input.value" type="default" size="small">Empty</n-tag>
                                <n-tag v-else-if="input.isValid === true" type="success" size="small">✓ Valid</n-tag>
                                <n-tag v-else-if="input.isValid === false" type="error" size="small">✗ Invalid</n-tag>
                                <n-tag v-else type="warning" size="small">⚠ Not validated</n-tag>
                            </div>
                        </div>
                    </div>
                    <div v-if="allInputsValid" class="summary-message">
                        <n-alert type="success" show-icon>
                            All inputs are valid! Ready to extract multi-language pairs.
                        </n-alert>
                    </div>
                </n-card>
                <n-card>
                    <div class="validation-grid">
                        ...
                        <n-button size="large" @click="downloadCSV" :disabled="extractedArray.length === 0"
                            type="success">
                            Download CSV
                        </n-button>
                    </div>
            ...
            <div class="json-input-section" v-for="(input, index) in jsonInputs" :key="index">
                <div class="input-header">
                    ...
                    <div class="input-actions">
                        <n-button size="small" @click="validateJson(index)"
                            :type="input.isValid ? 'success' : 'default'">
                            Validate
                        </n-button>
                        <n-button size="small" @click="clearInput(index)" type="warning">
                            Clear
                        </n-button>
                        <n-button size="small" @click="formatJson(index)" type="info">
                            Format
                        </n-button>
                    </div>
                </div>

                <n-input v-model:value="input.value" type="textarea"
                    :placeholder="`Enter JSON data for ${getLanguageName(index)}...`" :rows="12"
                    :status="input.isValid === false ? 'error' : undefined" @input="onInputChange(index)"
                    class="json-textarea" />
        ...
        <div v-if="extractedArray.length > 0" class="results-section">
            <div class="results-header">
                ...
                <div class="results-actions">
                    ...
                    <n-button size="small" @click="generateTestCase" type="primary"
                        :disabled="Object.keys(checkedKeys).length === 0">
                        Generate Test Case
                    </n-button>
                ...
                <n-tabs default-value="array" type="line">
                    ...
                    <n-tab-pane name="table" tab="Table View">
                        <div class="results-table">
                            <n-data-table :columns="tableColumns" :data="tableData" :pagination="{
                                pageSize: 20
                            }" :max-height="400" />
                        </div>
                    </n-tab-pane>
                    <n-tab-pane name="tree" tab="Tree View">
                        <div class="results-table">
                            <n-data-table :columns="treeColumns" :data="treeData" :cascade="false"
                                :children-key="'children'" :max-height="400" default-expand-all />
                        </div>
                    </n-tab-pane>
                    <n-tab-pane name="hash" tab="Hash Map">
                        <div class="results-content">
                            <pre
                                class="results-code">{{ JSON.stringify(Object.fromEntries(Array.from(extractedHashMap.entries())), null, 2) }}</pre>
                        </div>
                    </n-tab-pane>
                </n-tabs>
            </div>
        ...
        <n-modal v-model:show="showTestCaseModal" width="1000" @close="clearTestCaseForm">
            <n-card style="width: 1280px" title="Generate Test Case Modal" :bordered="false" size="huge" role="dialog"
                aria-modal="true">
                ...
                <n-flex vertical>
                    <n-form>
                        ...
                        <n-form-item label="条件 Condition">
                            <n-input v-model:value="testCaseContext" type="textarea" placeholder="Enter your text here"
                                rows="5" />
                        </n-form-item>
                        <n-form-item>
                            <n-button type="primary" :disabled="!testCaseScope || !testCaseContext || !testCaseLang"
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr>
                            <th v-for="(item, index) in testCaseHeader" :key="index"
                                style="border: 1px solid #ddd; padding: 8px; background-color: #f5f5f5;">
                                <span>{{ item }}</span>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="previewTestCase.length > 0">
                            <td v-for="(item, index) in previewTestCase" :key="index"
                                style="border: 1px solid #ddd; padding: 8px;">
                                <span>{{ item }}</span>
                            </td>
                        </tr>
                        <tr v-else>
                            <td :colspan="testCaseHeader.length"
                                style="border: 1px solid #ddd; padding: 8px; text-align: center; color: #999;">
...
// Reactive key to force re-render of validation summary
...
const initializeJsonInputs = async () => {
    try {
        ...
        validationKey.value++ // Force re-render of validation summary after initialization
...
const onInputChange = (index: number) => {
    ...
    jsonInputs[index].error = null
    validationKey.value++ // Force re-render of validation summary
}
...
const tableColumns = [
    ...
    },
    {
        ...
        render: (row: any) => h('span', {
            style: {
                backgroundColor: hasInvalidCharacters(row.lang1, 'japanese') ? '#fee2e2' : 'transparent',
                padding: '4px 8px',
                borderRadius: '4px',
    ...
    },
    {
        ...
        render: (row: any) => h('span', {
            style: {
                backgroundColor: hasInvalidCharacters(row.lang2, 'chinese') ? '#fee2e2' : 'transparent',
                padding: '4px 8px',
                borderRadius: '4px',
                display: 'inline-block',
                'white-space': 'pre-wrap'
    ...
    },
    {
        ...
        render: (row: any) => h('span', {
            style: {
                backgroundColor: hasInvalidCharacters(row.lang3, 'thai') ? '#fee2e2' : 'transparent',
                padding: '4px 8px',
                borderRadius: '4px',
                display: 'inline-block',
                'white-space': 'pre-wrap'
...
const isChinese = (text: string): boolean => {
    // Chinese character ranges: CJK Unified Ideographs, Extensions
...
const hasInvalidCharacters = (text: string, expectedLanguage: 'japanese' | 'chinese' | 'thai'): boolean => {
    if (!text || text.trim() === '') return false

    // Remove common punctuation and numbers that are acceptable in all languages
    const cleanText = text.replace(/[\s\d\p{P}\p{S}]/gu, '')

    // If no characters left after cleaning, it's valid
    if (!cleanText) return false

    switch (expectedLanguage) {
        case 'japanese':
            return !isJapanese(cleanText)
        case 'chinese':
            return !isChinese(cleanText)
        case 'thai':
            return !isThai(cleanText)
        default:
            return false
    }
}

const validateLanguageData = (data: [string, ...string[]][]): Array<{ key: string, issues: Array<{ index: number, language: string, text: string }> }> => {
    const languageNames = ['japanese', 'chinese', 'thai'] as const
    const validationResults: Array<{ key: string, issues: Array<{ index: number, language: string, text: string }> }> = []

    data.forEach(([key, ...labels]) => {
        const issues: Array<{ index: number, language: string, text: string }> = []

        labels.forEach((label, index) => {
            if (index < languageNames.length) {
                const expectedLanguage = languageNames[index]
                if (hasInvalidCharacters(label, expectedLanguage)) {
                    issues.push({
                        index,
        ...
        if (issues.length > 0) {
            validationResults.push({ key, issues })
        }
    })
...
// Reactive validation (for internal use if needed)
const validationResults = computed(() => validateLanguageData(extractedArray.value))

// Check if all inputs are valid
const allInputsValid = computed(() => {
    return jsonInputs.every(input => input.value && input.isValid === true)
})
